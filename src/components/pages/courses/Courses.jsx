...
const Courses = () => {
  ...
  const pageChangeHandler = async (pageNum) => {
    ...
    let data = await getDataByClick(`/Home/GetCoursesWithPagination?PageNumber=${pageCount}&RowsOfPage=6`)
    setTimeout(() => {dispatch(firstAddProduct(data.data.courseFilterDtos))}, 2000)
  }
  ...
  return (
    <div className="courses-holder flex justify-center mt-10"
      ...
      <div className="min-md:w-[75%] max-md:w-[90%] font-b-yekan flex flex-col items-center">
        <TopSection />
        <BottomSection>
          <PaginationData initialPageNum={1} totalNum={5} pageChange={pageChangeHandler}/>
        </BottomSection>
      </div>
    </div>
  );
