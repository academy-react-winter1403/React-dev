...
const TopSection = ({ data }) => {
  ...
  const profileCompletionPercentage = userProfileInfoData?.profileCompletionPercentage;

  return (
    <div
      className={`top-item-control w-full p-2 flex items-center relative justify-center gap-x-8
        max-lg:justify-between`}
      dir={isRTL ? "rtl" : "ltr"}
    >
      <div className={`right flex items-center ${isRTL ? "gap-x-5" : "gap-x-5"}`}>
        <motion.div
          initial={{ scale: 0, visibility: "hidden", opacity: 0 }}
          animate={{ scale: 1.1, visibility: "visible", opacity: 1 }}
          transition={{ duration: 0.4 }}
        >
          <StatusCard
            title={t("joinedCourses")}
            description={t("joinedDescription")}
            textColor={"#00B4AF"}
            titleChildren={
              <p className="text-[#00B4AF] text-[16px] text-start max-xl:text-center font-extrabold font-b-yekan">
                {myCourse && `${myCourse.totalCount} ${t("courses")}`}
              </p>
            }
          >
            <DashStudentIcon classNames={`w-[35px] h-[35px] relative top-[-19px]`} />
          </StatusCard>
        </motion.div>
        ...
        >
          <StatusCard
            ...
            titleChildren={
              <p className="text-[#FFB800] text-[16px] text-start max-xl:text-center font-extrabold font-b-yekan">
                {`${courseReserv?.length || 0} ${t("courses")}`}
              </p>
            }
          >
            <Basket classNames={`w-[35px] h-[35px] relative top-[-19px]`} />
