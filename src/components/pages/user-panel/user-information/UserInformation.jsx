...
//       {userProfileInfoData && (
//         <div
//           className="usre-information w-full p-7 rounded-xl bg-(--dashboard-userinfo)
        ...
        >
          ...
          <div className="top w-full flex justify-between mt-3.5 max-md:flex-col max-md:items-center max-md:gap-y-6">
            <div className="right w-[40%] max-md:w-full flex flex-col gap-y-9 max-md:gap-y-6">
              <TextCom
                userName={`${fName} ${lName}`}
                placeholderText={t("fullname")}
              />
              <TextCom userName={nationalCode} placeholderText={t("nationalCode")} />
              <TextCom userName={email} placeholderText={t("email")} />
              <TextCom
                userName={gender ? t("male") : t("female")}
                placeholderText={t("gender")}
              />
              <TextCom
                elemTitle={userAbout}
                userName={userAbout}
                placeholderText={t("about")}
                classStyles="w-full line-clamp-3 text-ellipsis overflow-x-hidden"
              />
            </div>
            <hr className="outline-0 border-0 w-[1px] h-[280px] bg-[#e4e4e4] max-md:hidden" />
            <div className="left w-[40%] max-md:w-full flex flex-col gap-y-6 max-md:gap-y-6">
              <TextCom userName={phoneNumber} placeholderText={t("phone")} />
              <TextCom userName={telegramLink} placeholderText={t("telegram")} />
              <TextCom userName={linkdinProfile} placeholderText={t("linkedin")} />
              <TextCom
                userName={homeAdderess}
                placeholderText={t("address")}
                classStyles="w-full line-clamp-3 text-ellipsis overflow-x-hidden"
              />
              <div className="maps-control flex justify-between items-start max-lg:flex-row max-xl:flex-col gap-y-2 max-md:flex-col max-md:items-center">
                <div className="maps-text-control flex flex-col gap-y-6 justify-start">
                  <TextCom userName={longitude} placeholderText={t("longitude")} />
                  <TextCom userName={latitude} placeholderText={t("latitude")} />
                </div>
                <Maps />
              </div>
            </div>
